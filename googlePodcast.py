...
def parsePage(result,lastdownload,channel,MAX_COUNT):
    ...
    count=0
    for whole_div in soup.select("div.oD3fme"):
        count=count+1
        if count>MAX_COUNT:    # 如果有10次，就直接结束
            return
        
        title=whole_div.select_one("div.LTUrYb div").get_text().strip()
        jsdata=whole_div.select_one('div.zlb4lf > div[jsname="fvi9Ef"]').get("jsdata").strip()
        audio_url=jsdata.split(";")[1]
...
stamp_path = "/opt/workspace/podcast/google.json"
google_url="https://podcasts.google.com/feed/"
